...
pub fn detect(input: &ReclassInput) -> Option<ReclassResult> {
    // Guard: zero capacity produces inf/NaN — treat as bad input
    if input.crew_size <= 0.0 || input.hrs_per_day <= 0.0 {
        return None;
    }
